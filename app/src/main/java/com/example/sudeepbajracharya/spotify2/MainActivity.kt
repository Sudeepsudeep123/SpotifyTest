...
//}
    ...
    private val CLIENT_ID = "0bbba50820ea409aa3d2ca36ae704167"
    private val REDIRECT_URI = "http://com.yourdomain.yourapp/callback"
    private var mSpotifyAppRemote: SpotifyAppRemote? = null
    private val mErrorCallback =
        ErrorCallback { throwable: Throwable? ->
            logError(
                throwable!!
            )
        }
    ...
    fun onToggleRepeatButtonClicked(view: View?) {
        mSpotifyAppRemote
            ?.playerApi
            ?.toggleRepeat()
            ?.setResultCallback { empty: Empty? ->
                Log.e("command Successful", "repeart")
            ...
            ?.setResultCallback { playerState: PlayerState ->
                if (playerState.isPaused) {
                    ...
                    mSpotifyAppRemote!!
                        ...
                        .pause()
                        .setResultCallback { empty: Empty? ->
                            Log.e("command Successful", "pause")
                        }
                        .setErrorCallback(mErrorCallback)
                }
            }
...
//    fun onSkipPreviousButtonClicked(view: View?) {
//        mSpotifyAppRemote
//            ?.playerApi
//            ?.skipPrevious()
//            ?.setResultCallback(CallResult.ResultCallback { data: Empty? ->
//                Log.e("command Successful", "skip previous")
//            })
//            ?.setErrorCallback(mErrorCallback)
//    }
    ...
    fun onSkipNextButtonClicked(view: View?) {
        mSpotifyAppRemote
            ?.playerApi
            ?.skipNext()
            ?.setResultCallback { data: Empty? ->
                Log.e("command Successful", "skip next")
            }
            ?.setErrorCallback(mErrorCallback)
    }
    fun onQueue(view: View?) {
        mSpotifyAppRemote
            ?.playerApi
            ?.queue("https://open.spotify.com/track/49FYlytm3dAAraYgpoJZux?si=0LuyqhXJQSmC4ZRgDHYiAA&utm_source=copy-link")
            ?.setResultCallback {
                Log.e("command Successful", "skip next")
            }
            ?.setErrorCallback(mErrorCallback)
    ...
    override fun onStart() {
        ...
        SpotifyAppRemote.connect(this, connectionParams, object : Connector.ConnectionListener {
            ...
            override fun onFailure(throwable: Throwable?) {
                Log.e("MainActivity", throwable?.localizedMessage, throwable);
            }
...
//        mSpotifyAppRemote?.playerApi?.play("spotify:playlist:37i9dQZF1DX2sUQwD7tbmL");
        mSpotifyAppRemote?.playerApi?.play("spotify:playlist:5QhCSa1JDnWWP7IUi1yPRd");
        ...
        mSpotifyAppRemote?.playerApi
                ...
                ?.setEventCallback { playerState: PlayerState ->
                    ...
                    if (track != null) {
                        findViewById<TextView>(R.id.txvNowPlaying).text = track.name.toString() + " by " + track.artist.name
